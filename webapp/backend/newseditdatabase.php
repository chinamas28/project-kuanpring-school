<?php
    ...
    if($_FILES["myFile"]["name"] == "") { //ไม่มีไฟล์อัพโหลด ไม่ต้องทำอะไร

        $sql = "UPDATE news SET newstypeid = '$type', topic = '$topic', content = '$content' WHERE newsid = '$newsid'";

    } else { // มีไฟล์อัพโหลดมา ต้องอัพ imagename ด้วย
        ...
        $sql = "UPDATE news SET newstypeid = '$type', topic = '$topic', content = '$content', imagename = '$filename' WHERE newsid = '$newsid'";
    ...
    if(mysqli_affected_rows($link) == 0) { // เช็คว่า result มีปัญหาไหม
        // บันทึกไม่สำเร็จ
        header("location:newsview.php?status=0");
    }
    else {
        // บันทึกสำเร็จ
        header("location:newsview.php?status=2");
    }
